--***********************************************************
--UPDATE Stored Procedure for Company table
--***********************************************************
GO
...
IF EXISTS (SELECT sys.objects.name FROM sys.objects INNER JOIN sys.schemas ON sys.objects.schema_id = sys.schemas.schema_id WHERE sys.objects.name = 'CompanyUpdate'    and sys.objects.type = 'P') 
DROP PROCEDURE [CompanyUpdate]
GO

CREATE PROCEDURE [CompanyUpdate] 
      @NewCompanyName varchar(8000)
     ...
     ,@NewInvoiceInitials varchar(8000)
     ,@NewAddUserID int
     ,@NewAddDate datetime
...
SET
     ...
     ,[EMail] = @NewEMail
     ,[GSTIN] = @NewGSTIN
     ,[InvoiceInitials] = @NewInvoiceInitials
...
GRANT EXECUTE ON [CompanyUpdate] TO [Public]
GO
