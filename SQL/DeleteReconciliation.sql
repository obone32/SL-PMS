--***********************************************************
--DELETE Stored Procedure for Reconciliation table
--***********************************************************
GO
...
IF EXISTS (SELECT sys.objects.name FROM sys.objects INNER JOIN sys.schemas ON sys.objects.schema_id = sys.schemas.schema_id WHERE sys.objects.name = 'ReconciliationDelete'    and sys.objects.type = 'P') 
DROP PROCEDURE [ReconciliationDelete] 
GO

CREATE PROCEDURE [ReconciliationDelete]
...
GRANT EXECUTE ON [ReconciliationDelete] TO [Public]
GO
